...
test({
	...
	test: (done) => {
		...
		const server = app.listen(port, () => {
			helpers.sendServerRequest(`/${name}`, 'GET', 200, '', {}, port).then((response) => {
				return helpers.sendServerRequest(`/${name}`, 'GET', 429, '', {}, port);
			}).then((response) => {
				setTimeout(() => {
					server.close();
...
test({
	...
	test: (done) => {
		...
		app.listen(port, () => {
			helpers.sendServerRequest(`/${name}`, 'GET', 200, '', {}, port, name).then(() => {
				done();
			}).catch(done);
		});
	}
...
test({
	...
	test: (done) => {
		...
		const portOne = getNextPort();
		const portTwo = getNextPort();
		...
		const serverOne = appOne.listen(portOne);
		const serverTwo = appTwo.listen(portTwo);

		setTimeout(() => {
			helpers.sendServerRequest(`/${name}`, 'GET', 200, '', {}, portOne).then((response) => {
				return helpers.sendServerRequest(`/${name}`, 'GET', 429, '', {}, portTwo);
			}).then((response) => {
				serverOne.close();
				serverTwo.close();
				assert.deepStrictEqual(typeof response.headers['retry-after'], 'string');
...
test({
	...
	test: (done) => {
		...
		app.listen(port, () => {
			helpers.sendServerRequest(`/${name}`, 'GET', 200, '', {}, port).then((response) => {
				return helpers.sendServerRequest(`/${name}`, 'GET', 200, '', {}, port);
			}).then((response) => {
				assert.equal(response.body.toString(), name);
