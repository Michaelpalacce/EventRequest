...
test({
	...
	test: (done) => {
		...
		const server = app.listen(port, () => {
			helpers.sendServerRequest(`/${name}`, 'GET', 200, '', {headerName: 'value'}, port).then((response) => {
...
test({
	...
	test: (done) => {
		...
		const server = app.listen(port, () => {
			helpers.sendServerRequest(`/${name}`, 'GET', 500, '', {headerName: 'value'}, port, '{"error":{"code":"app.general","message":"Some error occurred"}}').then((response) => {
				fileTransport.getWriteStream().end();
				setTimeout(() => {
					process.log = undefined;
...
test({
	...
	test: (done) => {
		...
		const server = app.listen(port, () => {
			helpers.sendServerRequest(`/${name}`, 'GET', 200, '', {
				headerName: 'value',
